...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
       ...
       BOOL ret = [_mapManager start:BaiduMapsKey  generalDelegate:self];
       if (!ret) {
           NSLog(@"地图启动失败");
       }else{
           NSLog(@"地图启动成功");
...
//       [self initJPushService:application WithOption:launchOptions];
       ...
       [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(tabSelect:) name:@"tabSelectIndex" object:nil];
       //环信客服
...
//    [UdStorage storageObject:@"" forKey:UserName];
