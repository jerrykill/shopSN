...
- (void)viewDidLoad {
    ...
    [self.navigationController.navigationBar setBarTintColor:__DefaultColor];
    self.title = @"客服";
       ...
       model.nickname = @"客服";
        ...
        if ([UdStorage getObjectforKey:UserHead]){
            if ([[UdStorage getObjectforKey:UserHead] isKindOfClass:[NSData class]]) {
                model.avatarImage = [UIImage imageWithData:[UdStorage getObjectforKey:UserHead]];
            }else{
                model.avatarImage = [UIImage imageWithData:[NSData
                                     dataWithContentsOfURL:[NSURL
                                             URLWithString:[NSString
                                         stringWithFormat:@"%@%@",HomeADUrl,[UdStorage getObjectforKey:UserHead]]]]];
            }
        }
    }
...
- (void)deleteAllMessages:(id)sender
{
    ...
    if ([sender isKindOfClass:[NSNotification class]]) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"提醒", @"Prompt") message:NSLocalizedString(@"确认删除聊天记录", @"please make sure to delete") delegate:self cancelButtonTitle:NSLocalizedString(@"取消", @"Cancel") otherButtonTitles:NSLocalizedString(@"确认", @"OK"), nil];
        [alertView show];
    }
}
