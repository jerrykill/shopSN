...
-(void)choosePut{
    NSLog(@"确认提交");
    if ([self checkNull]) {
        [JKHttpRequestService POST:@"Order/RefundSubmit" Params:@{@"order_id":_applyData.orderId,@"price":_applyData.price,@"case":_applyData.info,@"app_user_id":[UdStorage getObjectforKey:Userid]} NSArray:_applyData.goodArr key:@"refund" success:^(id responseObject, BOOL succe, NSDictionary *jsonDic) {
            if (succe) {
