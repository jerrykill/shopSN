...
- (void)getdata {
   ...
   [JKHttpRequestService POST:@"Index/poopClear" withParameters:nil success:^(id responseObject, BOOL succe, NSDictionary *jsonDic) {
       if (succe) {
           ...
           weakSelf.headAdArr = [YSParseTool getParseAD:data[@"top_img"]];
           weakSelf.activityArr = [YSParseTool getParseHotGoods:data[@"activity"]];
           NSArray *list = data[@"activity"];
           NSDictionary *dic = list.firstObject;
           weakSelf.endTime = dic[@"time"];
           weakSelf.classArr = [YSParseTool getParseClearance:data[@"poopClear"]];
           weakSelf.lastArr = [YSParseTool getParseHotGoods:data[@"last_clear"]];
           weakSelf.hotImgArr = [YSParseTool getParseAD:data[@"poopClear_ad"]];
